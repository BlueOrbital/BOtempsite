  ...
  props: TSProps,
    ...
    for (let i = 0; i < repeat - 1; i++) {
        ...
        >
            ...
            <React.Fragment
                key={`${element.length}-${i}-index-${index}`}
            >
                {index % 2 !== 0 && (
                <h2
                    className={`${fadeAnimationSelector[Math.floor(Math.random()*4)]} opacity-0 inline mx-2 font-thin min-w-fit overflow-clip}`}
                >
                    {message}
                </h2>
                )}
    ...
    useEffect(():void => {
        return (
            setContent(printDiv(props.repeat))
        );
    },[]);
